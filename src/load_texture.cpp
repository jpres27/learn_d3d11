...
void load_sky_texture(Texture_Info *texture_info)
{
    ...
    filenames[5] = "back.png";
    filenames[4] = "front.png";
...
for(int i = 0; i < 6; ++i)
{
        images[i].pixel_data = stbi_load(filenames[i],
                                    &image_width, 
                                    &image_height, 
                                    &image_channels, image_desired_channels);
        assert(images[i].pixel_data);
        images[i].pitch = image_width * 4;
}
        ...
        for(int i = 0; i < 6; ++i)
        {
            subresource_data[i].pSysMem = images[i].pixel_data;
            subresource_data[i].SysMemPitch = images[i].pitch;
            subresource_data[i].SysMemSlicePitch = 0;
        }
        hr = device->CreateTexture2D(&texture_desc, &subresource_data[0], &texture_info->texture);
        AssertHR(hr);
