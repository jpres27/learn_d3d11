...
void load_cube_mesh()
{
    ...
    init_data.pSysMem = cube_indices;

    hr = device->CreateBuffer(&bd, &init_data, &cube_index_buffer);
    AssertHR(hr);
...
void load_sphere_mesh(Sphere *sphere)
{
    ...
    D3D11_BUFFER_DESC bd = {};
    bd.Usage = D3D11_USAGE_DEFAULT;
    bd.ByteWidth = sizeof(DWORD)*360;
    bd.BindFlags = D3D11_BIND_INDEX_BUFFER;
    bd.CPUAccessFlags = 0;
    bd.MiscFlags = 0;

    D3D11_SUBRESOURCE_DATA init_data ={};
    init_data.pSysMem = sphere->indices;

    hr = device->CreateBuffer(&bd, &init_data, &sphere_index_buffer);
    AssertHR(hr);
...
void load_ground_mesh()
{
    ...
    hr = device->CreateBuffer(&bd, &init_data, &ground_index_buffer);
    AssertHR(hr);
